...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Esjis::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    # ShiftJIS regexp mark last m// or qr// matched
    sub Esjis::m_matched() {
        $last_s_matched = 0;
    }

    # ShiftJIS regexp mark last s/// or qr matched
    sub Esjis::s_matched() {
        $last_s_matched = 1;
    }
    ...
    @Esjis::m_matched = (qr/(?{Esjis::m_matched})/);
    @Esjis::s_matched = (qr/(?{Esjis::s_matched})/);
}
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Esjis::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
