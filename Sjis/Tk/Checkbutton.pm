package Sjis::Tk::Checkbutton;
######################################################################
#
# Sjis::Tk::Checkbutton - Tk Checkbutton widget for ShiftJIS script
#
...
sub configure {
    ...
    if (@argv == 0) {
        ...
        for my $config (@config) {
            if ($config->[0] =~ m/\A (?: -text | -onvalue | -offvalue ) \z/oxms) {
                my $text = $config->[4];
                push @config_cp932, [
                    $config->[0],
                    $config->[1],
        ...
        if ($argv[0] =~ m/\A (?: -text | -onvalue | -offvalue ) \z/oxms) {
            if (wantarray) {
        ...
        if (defined $argv{'-onvalue'}) {
            $argv{'-onvalue'} = Sjis::Encode::utf8($argv{'-onvalue'});
        }

        if (defined $argv{'-offvalue'}) {
            $argv{'-offvalue'} = Sjis::Encode::utf8($argv{'-offvalue'});
        }
...
sub cget {
    ...
    if ($argv =~ m/\A (?: -text | -onvalue | -offvalue ) \z/oxms) {
