...
if (join('', map {"($_)"} @_) eq "(‚“)(‚ˆ)(‚)(‚’)(‚”)(@)(‚”)(‚…)(‚“)(‚”)") {
    print qq{ok - 2 \@_ = split(//, "‚“‚ˆ‚‚’‚”@‚”‚…‚“‚”")\n};
}
else {
    print qq{not ok - 2 \@_ = split(//, "‚“‚ˆ‚‚’‚”@‚”‚…‚“‚”")\n};
}

# “Áê‚Èƒ}ƒbƒ`”í‰‰Zq " "(ƒXƒy[ƒX1ŒÂ•ª‚Ì’Êí‚Ì•¶š—ñ)‚ğg‚Á‚½ê‡
@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ");
if (join('', map {"($_)"} @_) eq "(‚)(‚“‚ˆ‚‚’‚”)(‚”‚…‚“‚”)") {
    print qq{ok - 3 \@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ")\n};
}
else {
    print qq{not ok - 3 \@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ")\n};
}

# æ“ª‚Ì‹ó”’‚ğc‚µ‚½‚¢ê‡
@_ = split(m/\s+/, "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ");
if (join('', map {"($_)"} @_) eq "()(‚)(‚“‚ˆ‚‚’‚”)(‚”‚…‚“‚”)") {
    print qq{ok - 4 \@_ = split(m/\\s+/, "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ")\n};
}
else {
    print qq{not ok - 4 \@_ = split(m/\\s+/, "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ")\n};
}

# ––”ö‚Ì‹ó”’‚ğc‚µ‚½‚¢ê‡
@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ", -1);
if (join('', map {"($_)"} @_) eq "(‚)(‚“‚ˆ‚‚’‚”)(‚”‚…‚“‚”)()") {
    print qq{ok - 5 \@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ", -1)\n};
}
else {
    print qq{not ok - 5 \@_ = split(" ", "   ‚ ‚“‚ˆ‚‚’‚”   ‚”‚…‚“‚”   ", -1)\n};
}
...
@_ = split(/F/);
if (join('', map {"($_)"} @_) eq "(‚h‚nD‚r‚x‚r)(‚Q‚Q‚T‚T‚T‚W)(‚X‚T|‚P‚O|‚O‚R)(|‚|‚“‚ˆ)(‚‚‚”‚‰‚‚‚‚Œ)") {
    print qq{ok - 7 \@_ = split(/F/)\n};
}
else {
    print qq{not ok - 7 \@_ = split(/F/)\n};
}

# 7.7.1.3 ŒÂ”ãŒÀ”í‰‰Zq‚ÌŠî–{
@_ = split(/F/, $text, 3);
if (join('', map {"($_)"} @_) eq "(‚h‚nD‚r‚x‚r)(‚Q‚Q‚T‚T‚T‚W)(‚X‚T|‚P‚O|‚O‚RF|‚|‚“‚ˆF‚‚‚”‚‰‚‚‚‚Œ)") {
    print qq{ok - 8 \@_ = split(/F/, \$text, 3)\n};
}
else {
    print qq{not ok - 8 \@_ = split(/F/, \$text, 3)\n};
}

# 7.7.2 ‹ó—v‘f
@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚W");
if (join('', map {"($_)"} @_) eq "(‚P‚Q)(‚R‚S)()(‚V‚W)") {
    print qq{ok - 9 \@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚W")\n};
}
else {
    print qq{not ok - 9 \@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚W")\n};
}

# 7.7.2.1 ––”ö‚Ì‹ó—v‘f
@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚WFFF");
if (join('', map {"($_)"} @_) eq "(‚P‚Q)(‚R‚S)()(‚V‚W)") {
    print qq{ok - 10 \@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚WFFF")\n};
}
else {
    print qq{not ok - 10 \@_ = split(m/F/, "‚P‚QF‚R‚SFF‚V‚WFFF")\n};
}

# 7.7.2.3 •¶š—ñ‚Ì—¼’[‚Å‚Ì“Áê‚Èƒ}ƒbƒ`
@_ = split(m/F/, "F‚P‚QF‚R‚SFF‚V‚W");
if (join('', map {"($_)"} @_) eq "()(‚P‚Q)(‚R‚S)()(‚V‚W)") {
    print qq{ok - 11 \@_ = split(m/F/, "F‚P‚QF‚R‚SFF‚V‚W")\n};
}
else {
    print qq{not ok - 11 \@_ = split(m/F/, "F‚P‚QF‚R‚SFF‚V‚W")\n};
}
...
if (join('', map {"($_)"} @_) eq "(@‚‚‚„@)(<‚a>)(‚–‚…‚’‚™@)(<‚e‚n‚m‚s@‚ƒ‚‚Œ‚‚’‚’‚…‚„>)(‚–‚…‚’‚™)(</‚e‚n‚m‚s>)(@‚‚•‚ƒ‚ˆ)(</B>)(@‚…‚†‚†‚‚’‚”)") {
    print qq{ok - 14 \@_ = split(/(<[^>]*>)/, "@‚‚‚„@<‚a>‚–‚…‚’‚™@<‚e‚n‚m‚s@‚ƒ‚‚Œ‚‚’‚’‚…‚„>‚–‚…‚’‚™</‚e‚n‚m‚s>@‚‚•‚ƒ‚ˆ</B>@‚…‚†‚†‚‚’‚”")\n};
}
else {
    print qq{not ok - 14 \@_ = split(/(<[^>]*>)/, "@‚‚‚„@<‚a>‚–‚…‚’‚™@<‚e‚n‚m‚s@‚ƒ‚‚Œ‚‚’‚’‚…‚„>‚–‚…‚’‚™</‚e‚n‚m‚s>@‚‚•‚ƒ‚ˆ</B>@‚…‚†‚†‚‚’‚”")\n};
}
...
if (($1 eq "DEF") and ($2 eq "HI")) {
    ...
    @_ = split(/,/,$b);
    if (($1 eq "DEF") and ($2 eq "HI")) {
        print qq{ok - 15 split ‚É•›ì—p‚ª‚È‚¢‚±‚Æ‚ÌŠm”F ($1)($2)\n};
    }
    else {
        print qq{not ok - 15 split ‚É•›ì—p‚ª‚È‚¢‚±‚Æ‚ÌŠm”F ($1)($2)\n};
    }
}
else {
    print qq{not ok - 15 split ‚É•›ì—p‚ª‚È‚¢‚±‚Æ‚ÌŠm”F ($1)($2)\n};
